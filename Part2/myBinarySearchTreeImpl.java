...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
			...
			if(level > 0 && !my_is_empty()){
				...
				count +=my_left_tree().my_count_at_level(level);
				count +=my_right_tree().my_count_at_level(level);
			}	
			return count;
	}
	...
	public boolean my_is_balanced(){
		...
		if (this.my_is_empty()== false){
			if (this.my_right_tree().my_length() == 0&& this.my_left_tree().my_length()==0)
				verdict =true;
			
				else if
				(!(this.my_right_tree().my_length()==this.my_left_tree().my_length() || this.my_right_tree().my_length()==(this.my_left_tree().my_length() +1) ||
						this.my_right_tree().my_length()+1)==this.my_left_tree().my_length()){
							verdict =false;
						
		boolean right = this.my_right_tree().my_is_balanced();
		boolean left = this.my_left_tree().my_is_balanced();
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if(!my_is_empty()){
			if(!my_left_tree().my_is_empty()){
				...
				count +=my_left_tree().my_count_smaller_nodes(key);
			...
			if(!my_right_tree().my_is_empty()){
				...
				count +=my_right_tree().my_count_smaller_nodes(key);
			}
		}
		return count;	
	}	
